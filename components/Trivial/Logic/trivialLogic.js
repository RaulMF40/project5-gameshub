...
import trivialTemplate from '../trivial'
import shuffleElements from '../../../tools/shuffleElements'
import { timeout as timer } from '../../../tools/timeout'
...
const getCardsInfo = async () => {
  ...
  //! la ha sacado de aqui https://opentdb.com/ que es un trivialdatabase

  questionsArray = await fetch(
  ...
  for (let i = 0; i < questionsArray.length; i++) {
    ...
    const shuffledAnswersArray = shuffleElements(answersArray)
...
const getNewQuestion = (
  ...
  if (availableQuestions.length === 0 || questionsCounter > MAX_QUESTIONS) {
    ...
    saveScoreBtn.addEventListener('click', (event) =>
      saveHighScore(event, resetButton)
    )
  } else {
    ...
    answers[0].firstChild.innerText = he.decode(
      availableQuestions[0].firstAnswer
    )
    answers[1].firstChild.innerText = he.decode(
      availableQuestions[0].secondAnswer
    )
    answers[2].firstChild.innerText = he.decode(
      availableQuestions[0].thirdAnswer
    )
    answers[3].firstChild.innerText = he.decode(
      availableQuestions[0].fourthAnswer
    )

    scoreText.innerText = score

    unselectedCorrectAnswer = answers.find(
      (answer) => answer.firstChild.innerText === correctAnswer
    )
...
const addListenersToAnswers = (
  ...
  answers.forEach((answer) => {
    answer.addEventListener('click', async (event) => {
      ...
      let classToApply =
        selectedAnswer.innerText === correctAnswer ? 'correct' : 'incorrect'
...
export const resetTrivialGame = (event) => {
  contentRefresh('trivial')
