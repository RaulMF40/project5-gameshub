...
const generateBoard = (resetButton) => {
  ...
  for (let r = 0; r < rows; r++) {
    let row = []
    const connect4Row = createBoardGrid(
      'rtc-connect4-board-row',
      `row-${r + 1}`
    )
    ...
    for (let c = 0; c < columns; c++) {
      row.push(' ')
      const connect4Box = createBoardGrid(
        'rtc-connect4-board-box',
        `row-${r + 1} | box-${c + 1}`
      )

      connect4Circle = createBoardGrid('rtc-connect4-board-circle', `${r}-${c}`)
      connect4Circle.addEventListener('click', (event) =>
        connect4Logic(event, connect4Circle, columns, rows, board, resetButton)
      )
...
const connect4Template = (event) => {
  ...
  const connect4GameDescription = createTitle(
    'h2',
    'rtc-connect4-description',
    'Red Turn'
  )
  const connect4Reset = createButton('rtc-connect4-reset', 'Reset', (event) =>
    resetConnect4Game(event, board, connect4Circle)
  )
